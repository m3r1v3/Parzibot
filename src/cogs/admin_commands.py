...
class AdminCommands(commands.Cog):
    ...
    async def adminhelp(self, interaction: discord.Interaction):
        ...
        if interaction.user.guild_permissions.manage_messages:
            ...
            await Message.error_msg(ctx, "Parzibot // Error", "You doesn't have permissions for executing this command")
    ...
    async def role(self, interaction: discord.Interaction, name: str, color: str):
        ...
        if interaction.user.guild_permissions.manage_messages:
            if Role().get_role(str(ctx.guild.id)) is not None:
                ...
                await Message.admin_msg(ctx, "Parzibot // Default Role Hadn't Set",
                                        "**Server Default Role** should be set for using this command")
        else:
            await Message.error_msg(ctx, "Parzibot // Error", "You doesn't have permissions for executing this command")
    ...
    async def giverole(self, interaction: discord.Interaction, member: discord.Member, role: discord.Role):
        ...
        if interaction.user.guild_permissions.manage_messages:
            ...
            await Message.admin_msg(ctx, "Parzibot // Give Role",
                                    f"**{role.name} Role** has been given to **{member}**")
        else:
            await Message.error_msg(ctx, "Parzibot // Error", "You doesn't have permissions for executing this command")
    ...
    @app_commands.command(name="defaultrole",
                          description="Set default role what will be giving to new members of Server")
    @app_commands.describe(role="Role what will be set as default")
    async def defaultrole(self, interaction: discord.Interaction, role: discord.Role):
        ...
        if interaction.user.guild_permissions.manage_messages:
            if Role().get_role(str(interaction.guild.id)) is None:
                ...
                await Message.admin_msg(ctx, "Parzibot // Default Role Set", "**Server Default Role** has been set")
            else:
                await Message.admin_msg(ctx, "Parzibot // Default Role Already Set",
                                        "**Server Default Role** already had been set")
        else:
            await Message.error_msg(ctx, "Parzibot // Error", "You doesn't have permissions for executing this command")

    @app_commands.command(name="defaultroleremove",
                          description="Remove default role what will be giving to new members of Server")
