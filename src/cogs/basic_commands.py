...
class BasicCommands(commands.Cog):
    ...
    async def about(self, interaction: discord.Interaction):
        await Message.basic_msg(interaction, "Parzibot // About", (
    ...
    async def clear(self, interaction: discord.Interaction, number: int = 5):
        if number:
            ...
            await Message.basic_msg(interaction, "Parzibot // Clear", f"Cleared **{number}** messages")
        else: await Message.error_msg(interaction, "Parzibot // Error", f"Cannot clear **{number}** messages")

    @app_commands.command(name="help", description="List of Parzibot basic commands")
    async def help(self, interaction: discord.Interaction):
        await Message.basic_msg(interaction, "Parzibot // Basic Commands", (
    ...
    async def ping(self, interaction: discord.Interaction):
        await Message.basic_msg(interaction, "Parzibot // Ping", f"Ping is `{round(self.bot.latency * 1000)}ms`")

    @app_commands.command(name="members", description="Members of current Text Channel")
    async def members(self, interaction: discord.Interaction):
        await Message.basic_msg(interaction, "Parzibot // Channel Members", "".join(f"\t**{member}**\n" for member in interaction.channel.members))
