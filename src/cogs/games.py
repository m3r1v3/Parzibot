...
class Games(commands.Cog):
    ...
    async def _8ball(self, ctx, *, question: str):
        ...
        await Message.msg(ctx, f"Question: `{question}`", f"**Answer:** `{random.choice(responses)}`")
    ...
    async def dice(self, ctx, value):
        ...
        if v1 + v2 == value: await Message.msg(ctx, "You won :D", f"The sum of values is **{v1 + v2}**. The values of dice is **{v1}** and **{v2}**")
            ...
            await Message.msg(ctx, "You guess value one of the dice :D", (
    ...
    async def help(self, ctx, command=None):
        ...
        if command is None:
            await Message.msg(ctx, "Game Commands", (
            ...
            await Message.msg(ctx, "**/8ball** command - The Ball of Predictions", (
                "**Syntax:** **/8ball** `question`\n"
                "**Options:** `question` - Your Question for The Ball of Predictions **(Required)**"))
        elif command == "dice":
            await Message.msg(ctx, "**/dice** command - The Game of Dice", (
                "**Syntax:** **/dice** `value`\n"
            ...
            await Message.msg(ctx, "**/gamehelp** command - The List of Parzibot Game Commands", (
                "**Syntax:** **/gamehelp** `command`\n"
                "**Options:** `command` - The Help Message for Specific Game Command **(Optional)**"))
        elif command == "getgame":
            await Message.msg(ctx, "**/getgame** command - Choice Random Game from Our List", "**Syntax:** **/getgame**")
        elif command == "whiteblack":
            await Message.msg(ctx, "**/whiteblack** command - The White/Black Game", (
    ...
    async def getgame(self, ctx):
        ...
        await Message.msg(ctx, "Special game by Parzibot", f"I advise you to play **{random.choice(responses)}**")
    ...
    async def white_black(self, ctx, color: str):
        ...
        if color == result: await Message.msg(ctx, "You won :D", f"Right color is `{result}`")
        else: await Message.error(ctx, "You lose :(", f"Right color is `{result}`")
